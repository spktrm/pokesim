...
from pokesim.data import DEFAULT_WORKER_INDEX, EVAL_MAPPING, MODEL_INPUT_KEYS
...
def run_environment_wrapper(
    worker_index: int, model: nn.Module, learn_queue: mp.Queue, eval_queue: mp.Queue
):
    try:
...
def run_environment(
    ...
    while True:
        ...
        while True:
            ...
            if worker_index < DEFAULT_WORKER_INDEX:
                actor_step = ActorStep(
                    policy=policy, action=action, rewards=env_step.rewards
                )
